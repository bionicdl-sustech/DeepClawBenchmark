...
class FullAPIExample
{
  private:
    ...
    void FreerunExample();
    void SoftwareTriggerExample(char* timestamp, char* state);
  ...
  public:
    FullAPIExample() {};
    ~FullAPIExample() {};
    void Run(char* timestamp, char* state);
};
...
void FullAPIExample::FreerunExample()
{
    ...
    std::vector <pho::api::PFrame> FrameList;
    ...
    int64_t timems_ = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
    std::cout << "Time cost: " << timems_-timems << std::endl;
    std::ofstream in;
    in.open("/home/bionicdl/git-projects-py2/flange_handeye_calibration/data/MotionX-20fps-flange/timestamp.txt",std::ios::trunc);


    for (std::size_t i = 0; i< FrameList.size(); i++)
    {
      const pho::api::FrameInfo &FrameInfo = FrameList[i]->Info;
      in<<i<<"\t"<< timestampList[i] <<"\n";
      std::cout << "Saving frame as " << i << ".ply" << std::endl;
      char name[256];
      sprintf(name, "/home/bionicdl/git-projects-py2/flange_handeye_calibration/data/MotionX-20fps-flange/%d.ply", i);
      FrameList[i]->SaveAsPly(name/*, You have multiple storing options*/);
    }
    FrameList.clear();
    in.close();
}

void FullAPIExample::SoftwareTriggerExample(char* timestamp, char* state)
{
    //Check if the device is connected
    if (!PhoXiDevice || !PhoXiDevice->isConnected())
    {
...
void FullAPIExample::ChangeSettingsExample()
{
    ...
    NewOutputSettings.SendTexture = false;
    //Send all outputs
...
void FullAPIExample::DataHandlingExample(pho::api::PFrame SampleFrame, char* timestamp, char* state)
{
    ...
    std::cout << "Saving frame as " << Filename << std::endl;
...
void FullAPIExample::Run(char* timestamp, char* state)
{
    try
    {
        ConnectFirstAttachedPhoXiDeviceExample();
...
//        ChangeSettingsExample();
        SoftwareTriggerExample(timestamp,state);
        // DataHandlingExample();
