...
def get_states():
    # rospy.init_node('AUBO_State_Getter')
    rospy.Subscriber("io_states", IOState, callback)

def set_states():
    rospy.wait_for_service('aubo_driver/set_io')
    global set_io
    set_io = rospy.ServiceProxy('aubo_driver/set_io', SetIO)
